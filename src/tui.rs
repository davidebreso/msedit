...
const KBMOD_FOR_WORD_NAV: InputKeyMod =kbmod::CTRL;
...
impl ButtonStyle {
    ...
    /// Draw a checkbox prefix: `[◼︎ Example Button]`
...
impl<'a> Context<'a, '_> {
    ...
    pub fn checkbox(&mut self, classname: &'static str, text: &str, checked: &mut bool) -> bool {
        ...
        self.styled_label_add_text(if *checked { "[◼︎ " } else { "[◻︎ " });
        self.styled_label_add_text(text);
    ...
    fn button_label(&mut self, classname: &'static str, text: &str, style: ButtonStyle) {
        ...
        if let Some(checked) = style.checked {
            self.styled_label_add_text(if checked { "◼︎ " } else { "  " });
        }
        // Label text
